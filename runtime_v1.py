...
def visuAdd():
    query1 = 'delete from LastRemoved where Location = ' + str(rcv_location())
...
def visuRemove(Location):
    ...
    query1 = 'delete from LastAdded where Location = ' + str(Location)
    cursor.execute(query1)
    db.commit()
...
def unstore_needed():
    ...
    if (result[0][0] != 0):
        unstore_needed_var = 1
    else:
        unstore_needed_var = 0
    return unstore_needed_var
...
try:
    while True:
        ...
        if mode == 1:
            print 'Unstore Mode. Please wait...'
            time.sleep(25)
